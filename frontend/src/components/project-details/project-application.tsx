...
export function ApplicationDialog({ open, onOpenChange,roleData }: { open: boolean; onOpenChange: (open: boolean) => void; roleData:RoleFormData[] }) {
  const projectId= roleData?.find(item => item.id !== null)?.id;
    ...
    <Dialog open={open} onOpenChange={onOpenChange} >
      <DialogContent className="sm:max-w-[600px] lg:max-w-[800px] xl:max-w-[1000px] bg-neutral-100 border-gray-200 text-gray-800 p-0 overflow-hidden rounded-lg w-[calc(100%-2rem)]">
        <DialogHeader className="border-b border-gray-100 pb-4 p-6">
          ...
          <DialogTitle className="text-xl sm:text-2xl font-bold mt-4 text-gray-800">Join Our Team</DialogTitle>
        ...
        <form onSubmit={handleSubmit} className="space-y-6 p-6 lg:p-8">
            ...
            <Select onValueChange={handleRoleChange} value={formData.role}>
              ...
              <SelectContent className="bg-neutral-100 border-gray-200 text-gray-800">
                {roleData && roleData.map((role)=>(
                    <SelectItem value={role.role || ""}>{role.role?formatData(role.role):""}</SelectItem>
          ...
          <div className="space-y-2 lg:col-span-2">
            ...
            <div className="mt-1 flex items-center">
              <label
                ...
                className={`flex items-center justify-center w-full px-4 py-3 border ${fileName ? "border-blue-200 bg-blue-50" : "border-gray-200 bg-neutral-100"} rounded-md shadow-sm text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none cursor-pointer transition-colors`}
              >
            ...
            {fileName && (
              <div className="mt-3 flex flex-wrap items-center gap-2 p-3 bg-gray-50 rounded-md border border-gray-100">
                <div className="flex items-center flex-grow">
                  ...
                  <span className="truncate text-sm text-gray-700 font-medium">{fileName}</span>
          ...
          <div className="pt-4 flex justify-end lg:col-span-2">
            <Button type="submit" className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded-md text-base">
              Submit Application
            </Button>
          </div>
        </form>
