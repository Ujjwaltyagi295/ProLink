...
export const getAllMyProjects = catchErrors(
  async (req: Request, res: Response) => {
    ...
    const ownedProjectIds = userProjects.map((project) => project.id);
    const memberProjectIds = memberProjects.map((mp) => mp.projectId);
    const projectIds = Array.from(
      new Set([...ownedProjectIds, ...memberProjectIds])
    );
...
export const getProjectsById = catchErrors(async (req, res) => {
  ...
  const [roles, techStack, members, applications] = await Promise.all([
    ...
    db
      .select()
      .from(projectTechStack)
      .where(eq(projectTechStack.projectId, projectId)),
    db
      .select()
      .from(projectMembers)
